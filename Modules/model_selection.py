...
def get_model():
    opt_paid_free = st.sidebar.radio("Choose Model", ["Free (Limited)","Free (using API)","Paid (using API)"], index=1, horizontal=True)
    if opt_paid_free == "Free (Limited)":
        ...
        if model_name == "GroqAI-llama-3.3-70b-versatile":
        ...
        elif model_name == "GroqAI-deepseek-r1-distill-llama-70b":
            groq_key = st.sidebar.text_input("🔑 Groq API Key", type="password")
            if groq_key:
